...
const DetailController = (props: { jobs: HistoryEntry[], courses: HistoryEntry[] }) => {
  ...
  const renderSkillContent = () => {
    ...
    const createSkillCard = (xp: Skill, index: number, minDateTime: number, maxDateTime: number) => {
      return (
        <div className="card" key={ index }>
          <h4>{ xp.names }</h4>
          <SkillBarStack key={ index } skill={ xp } minDateTime={ minDateTime }
    ...
    return (
      <div className="info hard-skills">
        ...
        .map((xp, index) => createSkillCard(xp, index, minDateTime, maxDateTime)) }
      </div>
    );
  };
  ...
  const renderDescription = () => {
    ...
    const createCertificates = () => {
      if (props.certificates && props.certificates.length > 0) {
        return (
          <div className="date-company-text certificate">

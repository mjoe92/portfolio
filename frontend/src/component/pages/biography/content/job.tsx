...
const jobContent = (t: (key: string) => string): HistoryEntry[] => [
  {
    ...
    description: (
      <>
        ...
        <ul>
          ...
          <li>
            { t("remote-artifact-repository") } ({ t("jfrog") })
          </li>
          <li>
    ...
    field: Field.P,
    certificates: getTranslatedCertificateNames(
      Certificate.FRONTEND_DEVELOPMENT_PROFESSIONAL_CERTIFICATE,
      Certificate.AZURE_900_CERTIFICATE,
      Certificate.AZURE_204_CERTIFICATE,
      Certificate.GITLAB_CI_CD_ASSOCIATE_CERTIFICATE
    )
  },
  {
    id: "innostudio",
    ...
    description: (
      <>
        <p>
          ...
          { TextLinkProvider.load([City.BUDAPEST, City.PEKING, City.SYDNEY, City.DUBNA], true) }
          { Constants.DOT }
        </p>
        <ul>
          <p className="sub-title">{ t("my-duties") }</p>
          { [
            [1, "construction-designer"],
            [2, "device-assembler"],
            [3, "device-installer"],
            [4, "process-developer"]
          ].map((duty, index) => (
